...
int VariableEmissivityTest(char Element, bool VaryEmissivity){
	clock_t begin = clock();
	// ********************************************************** //
	...
	if (VaryEmissivity) EmissivityModel = 'f';
	...
	std::string filename;
	if( VaryEmissivity ) 	filename = "Data/EmissivityNonConst.txt";
	else 			filename = "Data/EmissivityConst.txt";
