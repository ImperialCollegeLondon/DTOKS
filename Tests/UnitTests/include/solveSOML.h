...
double solveSOML(double TemperatureRatio, double MassRatio, double Ionization,
    double PlasmaFlowSpeed){
    ...
    double s2 = Ionization*Ionization*sqrt(PI)*
        erf(PlasmaFlowSpeed)/(2*PlasmaFlowSpeed);

    return TemperatureRatio*s1/s2-LambertW(sqrt(MassRatio*TemperatureRatio)*
        exp(TemperatureRatio*s1/s2)/s2);
}
