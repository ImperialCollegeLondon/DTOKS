...
double solveSMOML(double TemperatureRatio, double HeatCapacityRatio, double PlasmaFlowSpeed, double MassRatio){
	...
	return TemperatureRatio*s1/s2-LambertW(sqrt(2*PI*TemperatureRatio*(1+HeatCapacityRatio*TemperatureRatio))*exp(TemperatureRatio*s1/s2)/s2)+log(2*PI*(1+HeatCapacityRatio*TemperatureRatio)/MassRatio)/2;
}
