...
Model::Model( std::shared_ptr <Matter> const& sample, PlasmaData const& pdata, double accuracy )
		:Sample(sample),Pdata(pdata),Accuracy(accuracy){
	Mo_Debug("\n\nIn Model::Model( std::shared_ptr <Matter> const& sample, PlasmaData const& pdata ):Sample(sample),Pdata(pdata),Accuracy(accuracy)");
}
