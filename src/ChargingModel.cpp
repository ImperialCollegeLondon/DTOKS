...
Model(sample,pdata,accuracy){
    C_Debug("\n\nIn ChargingModel::ChargingModel(std::string filename, "
        << " float accuracy, std::array<bool,CMN> models, Matter *& sample, "
        << "PlasmaData *&pdata) : Model(sample,pdata,accuracy)\n\n");
    UseModel = models;
    CreateFile(filename);
}
...
ChargingModel::ChargingModel(std::string filename, float accuracy, 
std::array<bool,CMN> models, Matter *& sample, PlasmaGrid_Data &pgrid):
Model(sample,pgrid,accuracy){
    C_Debug("\n\nIn ChargingModel::ChargingModel(std::string filename, "
        << "float accuracy, std::array<bool,CMN> models, Matter *& sample, "
        << "PlasmaGrid_Data &pgrid) : Model(sample,pgrid,accuracy)\n\n");
    UseModel = models;
    CreateFile(filename);
}
...
void ChargingModel::Test(){
    ...
    std::cout << "\n"; std::cout << Sample->get_temperature() << "\t";
    std::cout << solveOML(DSec+DTherm,Sample->get_potential()) << "\t";
    ...
    if( (DSec + DTherm) >= 1.0 ){ 
        ...
        //!< Calculate the potential with the normal current balance including
        //!< electron emission
    ...
    std::cout << solveOML(0.0,Sample->get_potential())-
...
double ChargingModel::ProbeTimeStep()const{
    ...
    if( Pdata->ElectronDensity != 0 )
        timestep = Accuracy*sqrt((epsilon0*Me)/
            (2*PI*Pdata->ElectronDensity*pow(echarge,2)));
...
//!< bitstream/10044/1/32003/1/Thomas-D-2016-PhD-thesis.pdf
//!< or
...
double ChargingModel::solveSOML(double guess){
    ...
    double IonFluxDiff(0.0), ElectronFluxDiff(0.0), x1(0.0), dT(0.0);
    double dTdiff(0.0);
    int i(0);
    double guessinit = guess;
    do{
        //!< If first time, don't update guess. Avoids numerical instabilities.
        if( i > 0 ) 
            guess = x1;
        if( i == 1000 ){ //!< Too many loops! Something has gone wrong!
            std::cerr << "\nError in ChargingModel::solveSOML()! Newton Rhapson"
                << " Method didn't converge to accuracy: " << Accuracy;
            return 0.0;
        }

        if( guessinit >= 0.0 ){ //!< OML Electron flux, SOML Ion flux
            if( uz == 0.0 ){
                IonFluxDiff = Pdata->IonDensity*(IonThermalVelocity/
                    sqrt(2.0*PI))*Pdata->Z/Tau;
            }else{
                IonFluxDiff = (Pdata->IonDensity*IonThermalVelocity/
                    (2.0*sqrt(2.0)*uz))*Pdata->Z*erf(uz)/Tau;
            }
            ElectronFluxDiff = -OMLElectronFlux(fabs(guess));
            dT = Richardson*pow(Sample->get_temperature(),2)*
            ...
            if( uz == 0.0 ){
                IonFluxDiff = (Pdata->ElectronTemp/Pdata->IonTemp)*
                    SOMLIonFlux(guess);
            }else{
                IonFluxDiff = Pdata->IonDensity*
                    IonThermalVelocity*(1.0/(4.0*sqrt(2.0)*uz))*(Pdata->Z/Tau)*
                    (erf(uzp)+erf(uzm));
            }
            
            ElectronFluxDiff = -Pdata->ElectronDensity*
            ...
            dTdiff = (Pdata->ElectronTemp/Sample->get_temperature())*(dT-
                    ...
                    *exp((-echarge*Sample->get_workfunction()-
                    guess*Kb*Pdata->ElectronTemp)
                    /(Kb*Sample->get_temperature()))/echarge);
        }

        x1 = guess - ( (( 1.0-DeltaSec())*OMLElectronFlux(guess) - dT - 
            SOMLIonFlux(guess))
...
double ChargingModel::solveSMOML(double guess){
    ...
    do{
        ...
        if( i == 2000 ){
            std::cerr << "\nError in ChargingModel::solveSMOML()! Newton "
                << "Rhapson Method didn't converge to accuracy: " << Accuracy;
            return 0.0;
        }
        if( guessinit >= 0.0 ){  //!< OML Electron flux, SMOML Ion Flux
            if( uz == 0.0 ){
                IonFluxDiff = Pdata->IonDensity*(IonThermalVelocity/sqrt(2.0))*
                    (Pdata->Z/Tau);
            }else{
                IonFluxDiff = (Pdata->IonDensity*IonThermalVelocity/
                    (2.0*sqrt(2.0)*uz))*Pdata->Z*erf(uz)/Tau;
            ...
            ElectronFluxDiff = -Pdata->ElectronDensity*sqrt(Kb*
                Pdata->ElectronTemp/(2*PI*Me));
            
            dT = Richardson*pow(Sample->get_temperature(),2)*(1.0-guess
                    *(Pdata->ElectronTemp/Sample->get_temperature()))
                    *exp((-echarge*Sample->get_workfunction()-guess*Kb*
                    Pdata->ElectronTemp)/(Kb*Sample->get_temperature()))/
                    echarge;
            dTdiff = (Pdata->ElectronTemp/Sample->get_temperature())*(dT-
                    Richardson*pow(Sample->get_temperature(),2)
                    *exp((-echarge*Sample->get_workfunction()-
...
double ChargingModel::solvePHL(double guess){
    ...
    double Beta = Sample->get_radius()
        /(sqrt(PI*Kb*Pdata->ElectronTemp*Me/(2.0*echarge*echarge*
    ...
    do{
        ...
        if( guess >= 0.0 ){  //!< PHL Electron flux, SOML Ion Flux
            ...
            ElectronFluxDiff=Pdata->ElectronDensity*
                ...
                (d_A-d_A*i_star)-PHLElectronFlux(guess);
            IonFluxDiff = (Pdata->IonDensity*IonThermalVelocity/
                (2.0*sqrt(2.0)*uz))*Pdata->Z*erf(uz)/Tau;
            dT = Richardson*pow(Sample->get_temperature(),2)*
                exp(-echarge*Sample->get_workfunction()/
                (Kb*Sample->get_temperature()))/echarge;
            dTdiff = 0.0;
            x1 = guess - ( ((1.0-DeltaSec())*PHLElectronFlux(guess) - dT -
                SOMLIonFlux(guess))/((1.0-DeltaSec())*ElectronFluxDiff - dTdiff-
                IonFluxDiff ) );
        }else{  //!< OML Electron flux, SOML Ion Flux
            double uzp = uz+sqrt(-Pdata->Z*guess/Tau);
            double uzm = uz-sqrt(-Pdata->Z*guess/Tau);
...
double ChargingModel::solvePosOML(double a, double guess){
    ...
    double x1 = guess - ( (( 1-a)*(1+guess) - sqrt(b*C)*exp(-guess))/((1-a) + 
        sqrt(b*C)*exp(-guess) ) );
    
    while( fabs(guess-x1) > Accuracy ){
        guess = x1;
        x1 = guess - ( (( 1-a)*(1+guess) - sqrt(b*C)*exp(-guess))/((1-a) + 
            sqrt(b*C)*exp(-guess) ) );
    }
    return guess;
}
...
double ChargingModel::solveMOML(){
    ...
    double Arg = sqrt(2*PI*TemperatureRatio*
        (1+HeatCapacityRatio*TemperatureRatio))*exp(TemperatureRatio);
    if( std::isinf(exp(TemperatureRatio)) ){
        static bool runOnce = true;
        std::string Warning = "LambertW exp(TemperatureRatio) is infinite in ";
        Warning += "solveMOML()! Assuming Potential = 0.0";
        WarnOnce(runOnce,Warning);
        return 0;
    ...
    return -1.0*(TemperatureRatio
            -LambertW(Arg)+log(2*PI*
            (1+HeatCapacityRatio*TemperatureRatio)/MassRatio)/2);
}

//!< Solve the Modified orbital motion limited potential for large emitting dust 
//!< grains. See the paper by Minas and Nikoleta, equation (1) and (2)
...
double ChargingModel::solveMOMLWEM(double DeltaTot){
    ...
    double IonThermalVelocity = sqrt((Kb*Pdata->IonTemp)/Pdata->mi);
    double PlasmaFlowSpeed = (Pdata->PlasmaVel-Sample->get_velocity()).mag3()/
        IonThermalVelocity;
    double Delta_Phi_em = 0.5*log((2.0*PI/MassRatio)*
        (1.0+HeatCapacityRatio*TemperatureRatio)/pow(1.0-DeltaTot,2.0));
    ...
    if( std::isinf(exp(TemperatureRatio/Ionization+Delta_Phi_em/Ionization)) ){
        static bool runOnce = true;
        std::string Warning = "LambertW exp(TemperatureRatio/Ionization+";
        Warning += "Delta_Phi_em/Ionization) is infinite in solveMOMLWEM()!";
        Warning += " Assuming Potential = 0.0";
        WarnOnce(runOnce,Warning);
        return 0;
    }else if( std::isinf(Arg) || Arg < 0.0 ){
        std::cout << "\nLambertW(Arg == " << Arg << ")!";
    ...
    return -1.0*(TemperatureRatio/Ionization+Delta_Phi_em/Ionization-
        LambertW(Arg));
}

//!< Solve the Modified orbital motion limited potential for large emitting dust 
//!< grains.
//!< See the paper by Minas and Nikoleta, equation (10) with (1), (2), (5) & (9)
//!< N. Rizopoulou and M. Bacharis, Phys. Plasmas 25, (2018).
double ChargingModel::solveMOMLEM(){
    double Tau = Pdata->IonTemp/Pdata->ElectronTemp;
    double MassRatio = Pdata->mi/Me;
    double Chi = 2.0*Sample->get_temperature()/(Pdata->ElectronTemp);
    //!< Ratio of emitted electron density to sheath edge density
    double Delta = 0.01; 
    ...
    double PlasmaFlowSpeed = (Pdata->PlasmaVel-Sample->get_velocity()).mag3()/
        IonThermalVelocity;
    double Delta_Phi_em = 0.5*log((2.0*PI/MassRatio)*
        (1.0+HeatCapacityRatio*Tau)/pow(1.0-DeltaTot,2.0));
    ...
    if( std::isinf(exp(Tau/Ionization+Delta_Phi_em/Ionization)) ){
        static bool runOnce = true;
        std::string Warning = "LambertW exp(Tau/Ionization+Delta_Phi_em";
        Warning += "/Ionization) is infinite in solveMOMLWEM()!";
        Warning += " Assuming Potential = 0.0";
        WarnOnce(runOnce,Warning);
        return 0;
    }else if( std::isinf(Arg) || Arg < 0.0 ){
...
double ChargingModel::DeltaTherm()const{
    ...
    double dtherm = (Richardson*pow(Sample->get_temperature(),2)*
